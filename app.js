...
function renderItemsEditor() {
  ...
  body.querySelectorAll("input").forEach(input => {
    ...
    input.addEventListener("input", (event) => {
      ...
      const totalCell =
        row ? row.querySelector(".item-total") : null;
  ...
  const rows = items.map(item => {
    ...
    return `
      <tr>
        ...
        <td>
          ${data.currency} ${money(price)}
        </td>

        <td>
          ${data.currency} ${money(qty * price)}
        </td>
      </tr>
    `;
    ...
    <div class="invoice-top">
      ...
      <div>
        ...
        <div class="business-name">
          ${escapeHtml(
            data.businessName ||
            "Taller automotriz D&K Santa Cruz"
          )}
        </div>
        ...
        <div class="business-lines">
          ...
          <div>
            ${escapeHtml(data.businessCity)}
          </div>

          <div>
            Teléfono:
            ${escapeHtml(data.businessPhone)}
          </div>
...
function addItem() {
  ...
  const last =
    $("itemsBody")
      .querySelector("tr:last-child input");
 ...
 * ==========================================================
 *
 * jsPDF no siempre puede utilizar directamente:
 *
 *     "logo-referencia.png"
 *
 * mediante addImage().
 *
 * Esta función carga la imagen y la convierte
 * a una imagen Base64/Data URL.
 */
...
function addPdfHeader(doc, data, logoData) {
   ...
   * LOGO
   *
   * Aquí utilizamos logoData en lugar
 ...
 * ==========================================================
 *
 * Ahora la función es async porque necesitamos
 * cargar el logo antes de generar el documento.
 */
...
async function generatePdf() {
  ...
  try {
     ...
     * Simplemente continuamos sin logo.
     */
  ...
  doc.autoTable({
     ...
     * IMPORTANTE:
     *
     * Si autoTable genera varias páginas,
     * volvemos a dibujar el encabezado
     * incluyendo el logo.
     */
  ...
  /*
   * ========================================================
   * NUEVA PÁGINA SI NO CABE EL PIE
   * ========================================================
   */
  ...
  if (data.taxRate > 0) {
    ...
    doc.text(
      ...
      `IVA ${data.taxRate}%: ${currency} ${money(tax)}`,
...
document.addEventListener(
  ...
  () => {
    ...
    document
      .querySelectorAll(
        "input:not(.item-description):not(.item-qty):not(.item-price), select"
      )
      .forEach(el => {
